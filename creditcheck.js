...
// reverse array incase card numbers are not same length.  iterate through with odd indexes doubled.
const validateCred = (arr) => {
  let credArr = arr.reverse();
  let total = 0;
  ...
  for (let i = 0; i < credArr.length; i++) {
    ...
    if (credArr[i] > 9) {
      total = total + (credArr[i] - 9);
    } else {
      total += credArr[i];
    }
  }
  // evaluate total for true or false
  if(total % 10 === 0){
    return true;
  } else {
    return false;
...
// find companies with inalid cards, no duplicates in new array
